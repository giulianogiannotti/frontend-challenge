...
// Dinámicamente generar categorías
export const categories: Category[] = (() => {
  const categoryMap: Record<
    string,
    { name: string; icon: string; count: number }
  > = {
    all: { name: "Todos", icon: "apps", count: products.length },
    tech: { name: "Tecnología", icon: "devices", count: 0 },
    textile: { name: "Textil", icon: "checkroom", count: 0 },
    office: { name: "Oficina", icon: "work", count: 0 },
    home: { name: "Hogar", icon: "home", count: 0 },
  };
...
// Dinámicamente generar proveedores
export const suppliers: Supplier[] = (() => {
  const supplierMap: Record<string, { name: string; products: number }> = {
    "smart-gifts": { name: "Smart Gifts", products: 0 },
    "top-gifts": { name: "Top Gifts", products: 0 },
    "qr-code": { name: "QR Code", products: 0 },
  };

  products.forEach((p) => {
    if (supplierMap[p.supplier]) {
      supplierMap[p.supplier].products++;
    }
  });

  return Object.entries(supplierMap).map(([id, value]) => ({
    id,
    ...value,
  }));
})();
